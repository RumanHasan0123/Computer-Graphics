...
const char *fragmentShaderSource = "#version 330 core\n"
    ...
    "void main()\n"
    "{\n"
    "   FragColor = vec4(1.0f, 1.0f, 0.0f, 1.0f); // Cyan\n"
    "}\n\0";
...
int main()
{
    ...
    // --------------------
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "0432310005101065", NULL, NULL);//My name is Ruman hasan , so i named the window RumanHasan.
    ...
    while (!glfwWindowShouldClose(window)) //checks whether the user has clicked the close button on the window or triggered any event to close it.
    {
        ...
        glClearColor(0.0f, 1.0f, 1.0f, 1.0f); // Cyan  Color er Background er jonno RGB = (0, 1, 1) disi
...
void processInput(GLFWwindow *window) 
{
    if(glfwGetKey(window, GLFW_KEY_5) == GLFW_PRESS)//Window will be closed if The Key "5"(last digit Of My ID) pressed //close the window when the user pressed escape
